...
workflow consensus_nanopore {
    ...
    main:
        if (params.cds) {
            ...
            qualimap(consensus_nanopore_process.out.bams)
        }
}
...
process consensus_nanopore_process {
    ...
    output:
        path "${params.prefix}_${ref.simpleName}.bam", emit: bams
        path "${params.prefix}_${ref.simpleName}.consensus.fasta"
    """
    # variant call
    mini_align -t ${params.threads} -p ${params.prefix}_${ref.simpleName} -i $single -r $ref -m -f -M 2 -S 4 -O 4,24 -E 2,1
...
process qualimap {
    ...
    publishDir "${params.outdir}/consensus", mode: 'copy', saveAs: { filename -> "${bam.simpleName}.pdf"}
